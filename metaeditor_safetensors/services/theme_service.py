...
class ThemeService:
    ...
    when using system theme selection.
    """

    def __init__(self, app: Optional[QApplication] = None):
        if app is None:
            # For testing, create a minimal mock that has setStyleSheet method
        ...
        self._current_theme_preference: Optional[str] = None  # Track current preference
        ...
        self._system_theme_changed_callbacks: List[Callable[[str], None]] = []
        self._app = app
        self._cached_system_theme: Optional[SystemTheme] = None
        self._current_theme: Optional[Theme] = None
        self._available_themes: Dict[str, Theme] = {}
    ...
    def _update_file_watchers(self):
        """Update file watchers for current theme."""
    ...
    def remove_theme_changed_callback(self, callback: Callable[[str], None]):
        """Remove a theme changed callback."""
    ...
    def add_system_theme_changed_callback(self, callback: Callable[[str], None]):
        ...
        self._system_theme_changed_callbacks.append(callback)

    def remove_system_theme_changed_callback(self, callback: Callable[[str], None]):
        """Remove a system theme changed callback."""
        if callback in self._system_theme_changed_callbacks:
            self._system_theme_changed_callbacks.remove(callback)

    def _notify_theme_changed(self, theme_id: str):
        """Notify all callbacks about theme change."""
        for callback in self._theme_changed_callbacks:
            try:
                callback(theme_id)
            except Exception as e:
                logger.warning(f"Error calling theme changed callback: {e}")

    def _notify_system_theme_changed(self, theme_name: str):
        """Notify all callbacks about system theme change."""
        for callback in self._system_theme_changed_callbacks:
            try:
                callback(theme_name)
            except Exception as e:
    ...
    def _resolve_system_theme(self) -> str:
        ...
        if system_theme == SystemTheme.DARK:
            ...
            for theme_id, theme in self._available_themes.items():
                if (
                    theme_id != "system"
                    and theme.config.category == SystemTheme.DARK.value
                ):
                    return theme_id
            ...
            for theme_id, theme in self._available_themes.items():
                if (
                    theme_id != "system"
                    and theme.config.category == SystemTheme.LIGHT.value
                ):
                    return theme_id

        # Ultimate fallback (shouldn't happen if we have themes)
        logger.error("No themes available for system resolution")
        return ""
    ...
    def get_current_theme_preference(self) -> Optional[str]:
        """Get the current theme preference (e.g., 'system', 'dark', 'light')."""
    ...
    def has_theme(self, theme_id: str) -> bool:
        """Check if a theme ID exists (including 'system')."""
        return theme_id == "system" or theme_id in self._available_themes

    def apply_default_theme(self) -> bool:
        """Apply the default theme (system)."""
    ...
    def apply_theme(self, theme_identifier: str) -> bool:
        ...
        try:
            ...
            if theme_identifier == "system":
                resolved_theme_id = self._resolve_system_theme()
                logger.debug(f"System theme resolved to: {resolved_theme_id}")
                if not resolved_theme_id:
                    logger.error("System theme resolution failed - no themes available")
                    return False
    ...
    def _apply_theme_internal(
        ...
        try:
            ...
            # Notify callbacks with original preference (so UI knows "system" is selected)
            signal_theme_id = original_preference or theme.config.theme_id
            self._notify_theme_changed(signal_theme_id)
    ...
    def _start_system_theme_monitoring(self):
        ...
        with self._monitor_lock:
            ...
            self._monitoring_thread = threading.Thread(
                target=self._system_theme_monitor_loop, name="ThemeMonitor", daemon=True
            )
    ...
    def _system_theme_monitor_loop(self):
        ...
        try:
            ...
            def theme_change_callback(theme_name: str):
                ...
                # Notify callbacks (thread-safe)
                self._notify_system_theme_changed(theme_name)
                self._on_system_theme_changed(theme_name)
    ...
    def _on_system_theme_changed(self, theme_name: str):
        """Handle system theme change (called on background thread)."""
        try:
            # Only react if we're currently using system theme
            if self._current_theme_preference != "system":
                logger.debug("Ignoring system theme change - not using system theme")
                return

            # Clear cached system theme to force re-detection
            self._cached_system_theme = None

            # Re-apply system theme to pick up the new system theme
            logger.info("Re-applying system theme due to system theme change")
