...
class StylesheetService(QObject):
    ...
    def _apply_from_resources(self):
        ...
        if style_file.open(QIODevice.ReadOnly | QIODevice.Text):
            ...
            print(f"Warning: Could not load stylesheet from resources: {self._resource_path}")
    ...
    def _apply_from_file(self):
        ...
        try:
            ...
            print(f"[DEV] Stylesheet not found at {self._filesystem_path}, falling back to resources")
            self._apply_from_resources()
    ...
    def _setup_file_watcher(self):
        ...
        if not self._filesystem_path or not os.path.exists(self._filesystem_path):
            print(f"[DEV] Warning: Stylesheet file not found for watching: {self._filesystem_path}")
            return
        
        if self._watcher is None:
            self._watcher = QFileSystemWatcher([self._filesystem_path])
            self._watcher.fileChanged.connect(self._on_file_changed)
            print(f"[DEV] Live stylesheet reloading enabled, watching: {self._filesystem_path}")
