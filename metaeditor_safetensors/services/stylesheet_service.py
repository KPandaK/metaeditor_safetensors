...
class StylesheetService(QObject):
    ...
    def _apply_from_resources(self):
        ...
        if style_file.open(QIODevice.ReadOnly | QIODevice.Text):
            ...
            logger.warning(
                f"Could not load stylesheet from resources: {self._resource_path}"
            )

    def _apply_from_file(self):
        """Load stylesheet from filesystem (development mode)."""
        if not self._filesystem_path:
            logger.debug("No filesystem path provided, falling back to resources")
            self._apply_from_resources()
            return
        ...
        try:
            with open(self._filesystem_path, "r", encoding="utf-8") as f:
                ...
                logger.debug(f"Loaded stylesheet from {self._filesystem_path}")
        except FileNotFoundError:
            logger.debug(
                f"Stylesheet not found at {self._filesystem_path}, falling back to resources"
            )
            self._apply_from_resources()
    ...
    def _setup_file_watcher(self):
        ...
        if not self._filesystem_path or not os.path.exists(self._filesystem_path):
            logger.debug(
                f"Stylesheet file not found for watching: {self._filesystem_path}"
            )
            return
        ...
        if self._watcher is None:
            ...
            logger.debug(
                f"Live stylesheet reloading enabled, watching: {self._filesystem_path}"
            )

    def _on_file_changed(self):
        """Handle file change events for live reloading."""
        logger.debug("Stylesheet changed, reloading...")
        self._apply_from_file()
