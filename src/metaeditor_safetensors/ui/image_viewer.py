...
class ImageViewerWindow(BaseWindow):
    ...
    def _create_window(self):
        ...
        # Position the window
        self._position_window(window_width, window_height, screen_width, screen_height)
    ...
    def _position_window(self, window_width, window_height, screen_width, screen_height):
        parent_x = self.parent.winfo_x()
        parent_y = self.parent.winfo_y()
        parent_width = self.parent.winfo_width()
        parent_height = self.parent.winfo_height()
        
        # Try to position to the left of parent window
        left_x = parent_x - window_width - 20  # 20px gap
        
        # If it goes off-screen to the left, position to the right
        if left_x < 0:
            left_x = parent_x + parent_width + 20
            
            # If it goes off-screen to the right, center on screen
            if left_x + window_width > screen_width:
                left_x = (screen_width - window_width) // 2
        
        # Center vertically relative to parent, but keep on screen
        center_y = parent_y + (parent_height - window_height) // 2
        center_y = max(0, min(center_y, screen_height - window_height))
        
        self.window.geometry(f"{window_width}x{window_height}+{left_x}+{center_y}")
