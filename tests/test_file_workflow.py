...
def test_save_dispatches_write(
    ...
    assert call.kwargs["progress_callback"] is progress_cb
    wrapped_success = call.kwargs["success_callback"]

    config_service.add_recent_file.assert_not_called()

    wrapped_success("current.safetensors")
...
def test_save_as_updates_current_file_on_success(
    ...
    wrapped_success("new.safetensors")
